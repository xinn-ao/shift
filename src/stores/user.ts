...
export const useUserStore = defineStore('user', {
  ...
  getters: {
    authMenuList: (state): MenuItem[] => {
      ...
      const checkPermission = (menu: MenuItem, isParent: boolean): boolean => {
        // 先判断角色权限
        if(isParent && state.menus){
          return state.menus.includes(menu.path)
        }else if(state.subMenus){
          if(menu.path === '/shift/shiftTypes'){
            return state.businessType === '03' && state.subMenus.includes(menu.path)
          }else{
            return state.subMenus.includes(menu.path)
          }
        }else{
          return false
        }
      ...
      const res = copyRoutes.filter((parent) => {
        if (!checkPermission(parent,true)) return false
        if (parent.children) {
          parent.children = parent.children.filter((child) => checkPermission(child,false))
        }
        return true
      })

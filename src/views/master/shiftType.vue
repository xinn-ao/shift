    ...
    <div class="container">
        ...
        <el-card shadow="hover" class="search-card">
            ...
            <el-form inline label-width="55px">
                <el-form-item label="業態">
                    <el-select v-model="search.bizType" placeholder="--業態選択--" clearable style="width:160px">
                        <el-option v-for="item in bizList" :key="item.code" :label="item.name" :value="item.code" />
                    </el-select>
                </el-form-item>
                <el-form-item>
        ...
        <el-table :data="tableData" border style="width:100%;margin-top:10px">
            ...
            <el-table-column label="区分タイプ" width="120" align="center">
                <template #default="{ row }">
                    <el-select v-if="isEditMode || row.isNew" v-model="row.type" size="small" placeholder="タイプ選択"
                        style="width:90px" @change="calcWorkHour(row)">
            ...
            <el-table-column label="勤務開始時間" width="140" align="center" v-if="ssqSearch">
                <template #default="{ row }">
                    <el-input v-if="isEditMode || row.isNew" v-model="row.startTime" size="small" placeholder="HH:MM"
                        :disabled="row.type !== '01'" @blur="calcWorkHour(row)" />
            ...
            <el-table-column label="勤務終了時間" width="140" align="center" v-if="ssqSearch">
                <template #default="{ row }">
                    <el-input v-if="isEditMode || row.isNew" v-model="row.endTime" size="small" placeholder="HH:MM"
                        :disabled="row.type !== '01'" @blur="calcWorkHour(row)" />
            ...
            <el-table-column label="実働時間" width="120" align="center" v-if="ssqSearch">
                <template #default="{ row }">
                    <el-input v-if="isEditMode || row.isNew" v-model="row.workHour" size="small" placeholder="HH:MM"
            ...
            <el-table-column label="順番" width="80" align="center">
                <template #default="{ row }">
                    <el-input v-if="isEditMode || row.isNew" v-model.number="row.sort" size="small" type="number"
                        style="width:50px" />
